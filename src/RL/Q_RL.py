...
if __name__ == "__main__":
    ...
    try:
        ...
        if loaded_q_tables:
            ...
            # 获取相似表达式列表
            similar_exprs = get_similarity_exprs(initial_expr)

            if similar_exprs:  # 如果存在相似表达式
                print(f"找到 {len(similar_exprs)} 个相似表达式: {similar_exprs}")

                best_results = []
                for similar_expr in similar_exprs:
                    similar_expr = str(sympify(similar_expr))
                    if similar_expr in agent.q_tables:
                        print(f"\n使用相似表达式 {similar_expr} 的策略进行优化...")
                        optimized_expr, reward, actions = optimize_expression(agent, env, similar_expr, initial_expr)
                        best_results.append((optimized_expr, reward, actions))
                        print(f"使用 {similar_expr} 策略得到的结果: {optimized_expr}, 奖励: {reward:.2f}")
                    else:
                        print(f"相似表达式 {similar_expr} 没有可用的Q表，跳过")

                if best_results:  # 如果有成功的优化结果
                    best_expr, best_reward, action_sequence = max(best_results, key=lambda x: x[1])
                    print("\n从相似表达式策略中得到的最佳结果:")
                else:
                    print("\n没有可用的相似表达式策略，将使用当前表达式的策略")
                    best_expr, best_reward, action_sequence = optimize_expression(agent, env, expression_id,
                                                                                  initial_expr)
            else:
                print("没有找到相似表达式，将使用当前表达式的策略")
        ...
        print("未找到 Q 表文件，开始训练新策略")
        train_agent(env, agent, expression_id, episodes=500)
        save_q_table(agent.q_tables)
        best_expr, best_reward, action_sequence = optimize_expression(agent, env, expression_id, initial_expr)
    except Exception as e:
        print(f"加载 Q 表时出错: {e}，开始训练新策略")
        train_agent(env, agent, expression_id, episodes=500)
        save_q_table(agent.q_tables)
        best_expr, best_reward, action_sequence = optimize_expression(agent, env, expression_id, initial_expr)
