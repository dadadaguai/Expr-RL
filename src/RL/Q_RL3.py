...
from similarityUtils import test_get_similarity_exprs
...
def get_best_strategy_from_similar(agent, target_expr):
    """从相似表达式中获取最佳策略"""
    similar_exprs = test_get_similarity_exprs(target_expr)
    print(similar_exprs)
...
def hybrid_optimize(agent, env, expression_id, expression):
    """混合优化策略"""
    # 规范表达式ID
    expr_id = str(sympify(expression_id))

    # 首先尝试从相似表达式中获取最佳策略
    best_strategy, strategy_reward = get_best_strategy_from_similar(agent, expr_id)

    if best_strategy:
        print(f"\n找到相似表达式的策略: {best_strategy}, 历史奖励: {strategy_reward:.2f}")
        env.reset(expression)
    ...
    # 使用训练后的agent进行优化
    env.reset(expression)
    optimized_expr, reward, actions = optimize_expression(agent, env, expr_id, expression)
